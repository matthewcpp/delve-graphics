...
private:
	...
	void createInstance() {
		...
		if (enableValidationLayers) {
			if (!checkValidationLayerSupport()) {
				throw std::runtime_error("unsupported validation layer required.");
			}
	...
	void createLogicalDevice() {
		...
		// note that Vulkan no longer makes a distinction between instance and device validation layers.
		// following code is provided for compatibility with older implementations.
		// up to date implementations will ignore these fields.
		if (enableValidationLayers) {
	...
	bool checkValidationLayerSupport() {
		...
		// check that all our required validation layers are supported
		for (const auto requiredLayerName : requiredValidationLayers) {
			...
			if (layerProperty == availableLayerProperties.end()) {
				std::cout << "Validation layer: " << requiredLayerName << "not supported by current Vulkan driver" << std::endl;
				return false;
			}
		}
